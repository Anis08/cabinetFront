...
import { Calendar, Plus, Clock, Users, Bell, ChevronLeft, ChevronRight, X, User, Trash2 } from 'lucide-react'
...
const CalendarSimple = () => {
  ...
  const [resheduleOpen, setResheduleOpen] = useState(false)
  const [rescheduleForm, setRescheduleForm] = useState({ patientName: '', appointmentId: '', patientId: '', newDate: '', oldDate: '' })
  ...
  const rescheduleAppointment = async () => {
    const { appointmentId, patientId, newDate, oldDate } = rescheduleForm;
    if (!newDate || !patientId) {
      alert('Tous les champs sont obligatoires.');
      return;
    }

    try {
      let response = await fetch(`${baseURL}/medecin/rendez-vous/reschedule`, {
        method: 'PUT',
        body: JSON.stringify({
          rendezVousId: appointmentId, newDate, patientId, oldDate
        }),
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          "Content-Type": "application/json",
      ...
      if (!response.ok) {
        ...
        if (response.status == 401) {
          ...
          response = await fetch(`${baseURL}/medecin/rendez-vous/reschedule`, {
            method: 'PUT',
            body: JSON.stringify({
              rendezVousId: appointmentId, newDate, patientId, oldDate
            }),
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setAppointments((prev) => prev.map(apt => apt.id === data.rendezVous.id ? data.rendezVous : apt));
      }
    }
    catch (error) {
  ...
  const deleteAppointment = async (appointmentId) => {
    try {
      let response = await fetch(`${baseURL}/medecin/rendez-vous/delete/${appointmentId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      ...
      if (!response.ok) {
        ...
        if (response.status == 401) {
          ...
          response = await fetch(`${baseURL}/medecin/rendez-vous/delete/${appointmentId}`, {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
                        ...
                        {selectedDateAppointments.map((appointment, idx) => {
                          ...
                          return (
                            <motion.div
                              ...
                              className={`p-4 rounded-lg border-2 ${status === 'past' ? 'bg-red-50 border-red-200' :
                            ...
                            >
                              <div className="flex items-start justify-between mb-2">
                                ...
                                <span className={`text-xs px-2 py-1 rounded-full font-medium ${status === 'past' ? 'bg-red-100 text-red-700' :
                              ...
                              <div className='flex'>
                                <div className="space-y-1">
                                  ...
                                  <p className="text-sm text-gray-600">
                                    📅 {new Date(appointment.date).toLocaleDateString('fr-FR', { timeZone: 'UTC' })}
                                  </p>
                                ...
                                {status === 'future' && <div className='flex-grow flex justify-end items-center space-x-2'>
                                  <button onClick={() => { setRescheduleForm({ patientName: patient.fullName, appointmentId: appointment.id, patientId: patient.id, newDate: '', oldDate: appointment.date })
                                setResheduleOpen(true)
                              }}
                                    className=' border border-green-400 rounded-lg px-3 h-12 text-green-600 bg-green-200'>
                                    reprogrammer
                                  </button>

                                  <button onClick={() => deleteAppointment(appointment.id)}
                                   className='border border-red-400 rounded-lg size-12 text-red-600 bg-red-200 flex justify-center items-center'>
                        ...
                        <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg">
                          ...
                          {filteredPatients.map(patient => (
                            <div
                              ...
                              className={`cursor-pointer px-3 py-2 hover:bg-blue-50 transition-colors ${String(newRendezVousFormData.patientId) === String(patient.id)
            ...
            <AnimatePresence>
              {resheduleOpen && (
                ...
                >
                  <div
                    className="absolute inset-0 bg-black/50"
                    onClick={() => setResheduleOpen(false)}
                  />
                  <motion.div
                    ref={dialogRef}
                    tabIndex={-1}
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          Patient: {rescheduleForm.patientName}
                        </label>
                      </div>

                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          Ancienne date: {new Date(rescheduleForm.oldDate).toLocaleDateString('fr-FR')}
                        </label>
                      </div>
                      {/* Date input */}
                      <div>
                        ...
                        <input
                          ...
                          onChange={() => setRescheduleForm(prev => ({ ...prev, newDate: event.target.value }))}
                          className="w-full border border-gray-300 rounded-lg py-2 px-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          required
                        />
                      </div>
