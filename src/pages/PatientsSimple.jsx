...
const PatientsSimple = () => {
  ...
  const { patients, setPatients, averageAge, newPatientsThisMonth, patientsViewedThisWeek,
    setAverageAge, setNewPatientsThisMonth, setPatientsViewedThisWeek,
  todayAppointments, setTodayAppointments } = useData();
  ...
  const NewPatient = async () => {
    ...
    const { fullName, phoneNumber, gender, poids, taille, dateOfBirth, bio, maladieChronique, add } = newPatientFormData;
...
setNewPatientFormData({
    ...
    try {
      let response = await fetch(`${baseURL}/medecin/create-patient`, {
        method: 'POST',
        body: JSON.stringify({
          fullName, phoneNumber, gender, poids, taille, dateOfBirth, bio, maladieChronique, add
        }),
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
              ...
              <form onSubmit={e => {
                ...
                <div className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    name="add"
                    checked={newPatientFormData.add}
                    onChange={(e) => setNewPatientFormData((prev) => ({ ...prev, add: e.target.value }))}
                  />
