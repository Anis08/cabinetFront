...
const QueueSimple = () => {
  ...
  const { todayAppointments, setTodayAppointments, completedAppointments,
    setCompletedAppointments, setAveragePaid, setCaDay, setCaWeek,
    patients, setPatients, setAverageAge, setNewPatientsThisMonth,
    setPatientsViewedThisWeek, todayAppointmentsFetched, setTodayAppointmentsFetched } = useData();
  ...
  useEffect(() => {
    const getTodayAppointments = async () => {
      if (todayAppointmentsFetched) return;
      try {
        ...
        setTodayAppointmentsFetched(true);
      }
      catch (error) {
        return { error: 'Une erreur est survenue lors de la création du patient.' }
      }
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-semibold text-gray-900">{todayAppointments?.find(ap => ap.state === 'Scheduled') ? 1 : 0}</p>
              </div>
            </div>
          </div>
        </motion.div>
              ...
              >
                <div>
                  <label className="block text-sm font-medium mb-1" htmlFor="paye">
                    Payé <span className="text-red-500">*</span> <span className='text-gray-400 text-sm'>(Si pas encore defini)</span>
                  </label>
